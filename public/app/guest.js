...
(function () {
  ...
  function tile(entry, opts) {
    ...
    return html`<button class="tile ${opts.multi ? 'is-multi' : ''} ${blocked ? 'is-blocked' : ''}"
      ...
      ${raw(meta.length && !blocked ? '<span class="tile-meta">' + PP.escapeHtml(meta.join(' \u00b7 ')) + '</span>' : '')}
      ${raw(blocked ? '<span class="tile-warn" id="clash-' + entry.id + '">has ' + PP.escapeHtml(clash.map(allergenName).join(', ')) + '</span>' : '')}
    </button>`;
  }
    ...
    var bowls = state.bowls.map(function (bowl, i) {
      ...
      return html`<div class="bowl">
        ...
        <div style="text-align:right">
          <div class="bowl-price">${PP.money(bowlPrice(bowl))}</div>
    ...
    return html`
      ...
      <div class="stack">
        ...
        <div class="card stack">
          ...
          <div class="summary-line"><span>Cook time</span><span>${PP.humanMins(estimate.cookEstimateSec)}</span></div>
          <div class="summary-total"><span>Total</span><span>${PP.money(estimate.subtotal)}</span></div>
  ...
  /** Ask the server to price and time the order; it owns the real numbers. */
  async function refreshEstimate() {
    try {
      var res = await PP.api('/api/estimate', { method: 'POST', body: { lines: state.bowls } });
      estimate = res;
    } catch (err) {
      // Fall back to a local subtotal so the review screen still shows a price.
      estimate = {
        cookEstimateSec: 0,
        promiseSec: 0,
        subtotal: state.bowls.reduce(function (a, b) { return a + bowlPrice(b); }, 0),
      };
    }
  }
  ...
  function screenSent() {
    ...
    if (order.status === 'queued' || order.status === 'cooking') {
      var remaining = Math.max(0, (new Date(order.promisedReadyAt).getTime() - PP.now()) / 1000);
      eta = html`<p class="eta">Ready in <strong>${PP.humanMins(remaining)}</strong></p>`;
    }
    ...
    return html`
      <div class="sentwrap stack">
        ...
        <div class="card">
          ...
          <div class="summary-total"><span>Total</span><span>${PP.money(order.totalPrice)}</span></div>
        </div>
      </div>`;
  }
  ...
  var actions = {
    ...
    submitMember: async function () {
      ...
      render({ keepScroll: true });
      try {
        var res = await PP.api('/api/members/lookup', {
          method: 'POST', body: { memberNumber: state.memberDigits },
        });
        state.member = res;
    ...
    send: async function () {
      ...
      render({ keepScroll: true });
      try {
        var res = await PP.api('/api/orders', {
          method: 'POST',
          body: {
            ...
            tag: state.boot.tag ? state.boot.tag.id : null,
            source: 'qr',
  ...
  function watchOrder(orderId) {
    if (state.stopStream) state.stopStream();
    state.stopStream = PP.stream({
      order: orderId,
      onEvent: function (type, data) {
        if (!data.order || data.order.id !== orderId) return;
        var was = state.order ? state.order.status : null;
        state.order = data.order;
        if (state.screen === 'sent') render({ keepScroll: true });
        if (was !== 'ready' && data.order.status === 'ready') {
          PP.chime('runner');
  ...
  async function boot() {
    ...
    // The QR code encodes the table: /t/<tag>
    var match = location.pathname.match(/^\/t\/([A-Za-z0-9_-]+)/);
    var tag = match ? match[1] : null;

    try {
      state.boot = await PP.api('/api/bootstrap' + (tag ? '?tag=' + encodeURIComponent(tag) : ''));
    } catch (err) {
      stage.innerHTML = '<div class="step-head"><h1 class="step-title">We cannot reach the kitchen</h1>' +
        '<p class="step-sub">Check the wifi and reload, or ask your server to take the order.</p></div>';
      return;
    }
